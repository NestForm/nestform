...
(function () {
	...
	function onReady() {
		...
		if (enable) {
			enable.addEventListener('change', function (event) {
				...
				if (enable.checked && !canMulti) {
					...
					if (typeof window.nestformOpenPro === 'function') {
						window.nestformOpenPro({
							title: (window.nestformPro && window.nestformPro.i18n && window.nestformPro.i18n.stepsTitle) || '',
							text: (window.nestformPro && window.nestformPro.i18n && window.nestformPro.i18n.stepsText) || '',
							viz: true,
		...
		if (addStepBtn) {
			addStepBtn.addEventListener('click', function () {
				...
				if (!canMulti) {
					if (typeof window.nestformOpenPro === 'function') {
						window.nestformOpenPro({
							title: (window.nestformPro.i18n && window.nestformPro.i18n.stepsTitle) || '',
							text: (window.nestformPro.i18n && window.nestformPro.i18n.stepsText) || '',
							viz: true,
							list: ['Multi-step wizard', 'Branch rules', 'Higher completion rate'],
						});
					}
		...
		root.addEventListener('click', function (event) {
			...
			var proField = event.target.closest('[data-nestform-pro-field]');
			if (proField && root.contains(proField)) {
				closeAllAddMenus(root);
				if (typeof window.nestformOpenPro === 'function') {
					var ptype = proField.getAttribute('data-nestform-pro-field') || '';
					var pi18n = (window.nestformPro && window.nestformPro.i18n) || {};
					if (ptype === 'calculated') {
						window.nestformOpenPro({
							title: pi18n.calcTitle || '',
							text: pi18n.calcText || '',
							list: ['Formula fields', 'Live front-end recalculation', 'Server-side verification'],
						});
					} else if (ptype === 'repeater') {
						window.nestformOpenPro({
							title: pi18n.repeaterTitle || '',
							text: pi18n.repeaterText || '',
							list: ['Repeating groups', 'Nested repeaters', 'Mail template loops'],
						});
					} else {
						window.nestformOpenPro({
							title: pi18n.fieldTitle || '',
							text: pi18n.fieldText || '',
							list: ['Rating', 'Signature', 'NPS', 'Scale', 'Ranking'],
						});
					}
				}
				return;
			...
			var proWebhook = event.target.closest('[data-nestform-pro-webhook]');
			if (proWebhook && root.contains(proWebhook)) {
				if (typeof window.nestformOpenPro === 'function') {
					window.nestformOpenPro({
						title: (window.nestformPro && window.nestformPro.i18n && window.nestformPro.i18n.webhookTitle) || '',
						text: (window.nestformPro && window.nestformPro.i18n && window.nestformPro.i18n.webhookText) || '',
